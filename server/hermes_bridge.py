...
- 任务编排：query 包含多个意图，或需要先查榜单/搜索/解析实体再做后续动作。例如“找一些最近最火的歌”“最近的新歌里挑几首适合跑步的”，应先 chart_query，再按需要 music_search 或 music_recommend。地域/身份/群体 + 新歌/最新/最近/近期/当下热门这类 query，通常应先用 ai_search 解析范围和候选实体，再用 music_search 查这些实体的新歌。
